 ...
 *
 * Stellt alle Informationen bereit, die fuer den kreislaufgerechten Rueckbau
 * einer BSP-Platte am Ende des Lebenszyklus benoetigt werden: Hersteller,
 * Abmessungen, Materialherkunft, Stofflichkeit und Leim, Materialverwertung
 * sowie Verbindungstechnik -- in sechs aufklappbaren Kategorien, darunter der
 * Downloadbereich der zugehoerigen Dokumente.
 *
 * Die Informationsanforderungen werden in services/deconstructionMapper.ts
 ...
 * laut Awf-Vorgabe Teil des Anwendungsfalls.
 */

interface DeconstructionViewProps {
  productId: string;
  product: Product | null;
  /** Rohe Abfrageergebnisse -- ERP-Panel, Leistungserklaerung, Klebstoff. */
...
/** Kennzeichnung fehlender Merkmale -- Wortlaut wie im Herkunftsnachweis. */
const NO_DATA = 'Keine Daten verfügbar';

/**
 * Eine Merkmalszeile.
 *
 * Fehlende Merkmale tragen ihre Begruendung als Titel und einen gestrichelten
 * Unterstrich, damit der Unterschied zwischen "nicht erhoben" und "nicht
 * vorgesehen" beim Nachfragen sichtbar wird, ohne die Liste zu ueberladen.
 */
function FieldRow({ field }: { field: DeconstructionField }) {
      ...
      <dd className="flex-1 min-w-0">
        ...
        {field.note && (
          /* Begruendung, warum ein Wert fehlt bzw. woher er stammt.
             ...
             ein Kontrastverhaeltnis von 3.5:1 und liegt damit unter der
             WCAG-Schwelle von 4.5:1 fuer kleinen Text -- auf dem dunklen
             Blau war das kaum lesbar. night-300 kommt auf 5.7:1. */
...
export function DeconstructionView({
  productId,
  product,
  productData,
  onBack,
  onScanClick,
}: DeconstructionViewProps) {
  // Wie in der Visualisierung: eine Kategorie zur Zeit offen.
  const [openCategory, setOpenCategory] = useState<string | null>(null);

  const deconstruction = useMemo(
    () => mapToDeconstruction(productData, product),
    [productData, product],
  );
      ...
      <div className="flex-1 bg-night-900">
        <div className="max-w-2xl mx-auto px-4 sm:px-6 py-6 sm:py-8">
          ...
          <h1 className="text-3xl sm:text-4xl font-extrabold tracking-tight text-white mb-2">
            Rückbaubarkeit <span className="text-acid-400">&amp; Zirkularität</span>
          </h1>
          <div className="bg-night-700/50 border border-white/5 rounded-2xl p-6 mt-6">
            <p className="text-sm text-night-300">
              Für die Rückbauinformationen wird ein Bauteil benötigt. Scannen Sie
              eine Platte oder geben Sie die Produkt-ID ein.
            </p>
            <button
      ...
      <div className="flex-1 bg-night-900">
        <div className="max-w-2xl mx-auto px-4 sm:px-6 py-6 sm:py-8">
          ...
          <div className="space-y-4">
            ...
            >
              {/* Reales Foto des Bauteils (Visualisierung S. 1: Bild links
                  neben "BAUTEIL / BSP-Wand"). Fehlt das Foto, zeigt die
                  Kachel die Illustration -- siehe productImageService. */}
              ...
              <span className="min-w-0">
                ...
                <span className="block text-xl font-extrabold text-white truncate">
                  {deconstruction.componentName ?? 'BSP-Platte'}
                </span>
                {deconstruction.componentType && (
            ...
            {/* Die sechs Kategorien zum Aufklappen */}
            <motion.section
              initial={{ opacity: 0, y: 16 }}
              animate={{ opacity: 1, y: 0 }}
              ...
              className="space-y-2"
            >
              {deconstruction.categories.map((category) => {
            ...
            >
              {deconstruction.coverage.filled} von {deconstruction.coverage.total} Angaben
